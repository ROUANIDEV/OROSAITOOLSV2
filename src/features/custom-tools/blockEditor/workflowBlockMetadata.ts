...
export function getBlockInputDetails(
  ...
  switch (block.type) {
    case "file.glob":
      return [
        `Folder: ${formatInputReference(block.config.rootInput, inputs, "folder input")}`,
        `Pattern: ${String(block.config.pattern ?? "**/*")}`,
      ];
    case "file.read":
      return [
        `File: ${formatInputReference(block.config.sourceInput, inputs, "file input")}`,
      ];
    case "text.template":
    ...
    case "file.appendText":
      return [
        `Target: ${formatInputReference(block.config.targetInput, inputs, "file input")}`,
      ];
    case "safety.confirm":
...
export function getConnectionLabel(from: CustomToolBlock, to: CustomToolBlock) {
...
export function getConnectionClassName(from: CustomToolBlock, to: CustomToolBlock) {
  if (from.type.startsWith("safety.") || to.type.startsWith("safety.")) {
