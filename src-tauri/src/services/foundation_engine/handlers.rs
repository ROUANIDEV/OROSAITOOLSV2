...
pub fn execute_block(
    ...
    let start_error_count = context
        .diagnostics
        .iter()
        .filter(|diagnostic| matches!(diagnostic.severity, super::types::FoundationDiagnosticSeverity::Error))
        .count();
...
fn validate_identifier(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock, field: &str, value: &str) -> bool {
...
fn execute_variable_create(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    if let Some(value) = coerce_typed_value(context, &block.id, "initialValue", &data_type, resolved) {
        ...
        context.outputs.insert(block.id.clone(), json!({ "value": value, "reference": name }));
        context.trace(
            block.id.clone(),
            block.block_type.clone(),
...
fn execute_variable_assign(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    context.variables.insert(name.clone(), value.clone());
    context.outputs.insert(block.id.clone(), json!({ "assignedValue": value }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn execute_constant_create(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    if let Some(value) = coerce_typed_value(context, &block.id, "value", &data_type, resolved) {
        ...
        context.outputs.insert(block.id.clone(), json!({ "constant": value, "reference": name }));
        context.trace(
            block.id.clone(),
            block.block_type.clone(),
            FoundationRuntimeTraceStatus::Executed,
...
fn execute_expression_value(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    if let Some(value) = coerce_typed_value(context, &block.id, "expression", &data_type, value) {
        context.outputs.insert(block.id.clone(), json!({ "result": value }));
        context.trace(
            block.id.clone(),
            block.block_type.clone(),
            FoundationRuntimeTraceStatus::Executed,
...
fn execute_collection_array(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    match value {
        Value::Array(items) => {
            context.outputs.insert(block.id.clone(), json!({ "array": items }));
            context.trace(
                block.id.clone(),
                block.block_type.clone(),
                FoundationRuntimeTraceStatus::Executed,
...
fn execute_collection_list(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    match value {
        Value::Array(items) => {
            context.outputs.insert(block.id.clone(), json!({ "list": items }));
            context.trace(
                block.id.clone(),
                block.block_type.clone(),
                FoundationRuntimeTraceStatus::Executed,
        ...
        _ => context.error(
            ...
            Some("Use [] or [\"a\", \"b\"].".to_string()),
        ),
    }
}

fn execute_collection_dictionary(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    let entries = block.config.get("entries").cloned().unwrap_or(Value::Object(Map::new()));
    ...
    context.outputs.insert(block.id.clone(), json!({ "dictionary": dictionary }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn execute_collection_set(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    context.outputs.insert(block.id.clone(), json!({ "collection": collection }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn plan_scope(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    context.outputs.insert(block.id.clone(), json!({ "scope": namespace }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Planned,
...
fn execute_function_define(context: &mut FoundationRuntimeContext, block: &FoundationRuntimeBlock) {
    ...
    context.outputs.insert(block.id.clone(), json!({ "function": name }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn execute_function_call(
    ...
    context.outputs.insert(block.id.clone(), json!({ "result": context.outputs.clone(), "returnType": function.return_type }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn execute_if(
    ...
    context.outputs.insert(block.id.clone(), json!({ "condition": truthy, "branch": if truthy { "true" } else { "false" } }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn execute_switch(
    ...
    context.outputs.insert(block.id.clone(), json!({ "value": value, "matchedCase": matched_case }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn execute_for_each_loop(
    ...
    let index_name = string_value(&block.config, "indexName").unwrap_or_else(|| "index".to_string());
    if !validate_identifier(context, block, "itemName", &item_name) || !validate_identifier(context, block, "indexName", &index_name) {
        return;
    }
    ...
    match items {
        ...
        Value::Object(map) => {
            for (index, (key, item)) in map.into_iter().enumerate() {
                ...
                context.variables.insert(item_name.clone(), json!({ "key": key, "value": item }));
                context.variables.insert(index_name.clone(), json!(index));
                execute_body_block_ids(context, &body_ids, block_lookup, options);
                iteration_count += 1;
            }
    ...
    context.outputs.insert(block.id.clone(), json!({ "iterations": iteration_count }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
...
fn execute_while_loop(
    ...
    let condition = string_value(&block.config, "condition").unwrap_or_default();
    let max_iterations = number_value(&block.config, "maxIterations", options.max_loop_iterations as f64) as usize;
    ...
    context.outputs.insert(block.id.clone(), json!({ "iterations": iteration_count, "completed": true }));
    context.trace(
        block.id.clone(),
        block.block_type.clone(),
        FoundationRuntimeTraceStatus::Executed,
